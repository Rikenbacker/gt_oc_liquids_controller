...
while true do
  // Если НЕ "Можно загребать из входа"	 или НЕ "Устаканился вход" то сканировать вход на предмет устаканивания
  // Если "Можно загребать из входа" И "Устаканился вход", то начать перекладывать в выход
  // После первой итерации выкладки вещей врубается "Можно забирать рецепт" и не вырубается до момента опустошения взодного и выължного сундука.
  // После опустошения сундука вырубается "Можно загребать из входа" и не врубается до момента пока не погаснет "Можно забирать рецепт"
  // "Можно забирать рецепт" гаснет после опустошения выходного сундука и врубается "Можно загребать из входа"
  // При погасшем "Можно загребать из входа" проверяетсся законченность работы химки и врубается "Вытащить схему"
  // Не забыть про лаг запуска химки после выкладки рецепта, так как может получиться что химка ещё не запускалась, а схема уже забралась.
  if mode == 0 then
    mode = waitForWork()
  end
  if mode == 1 then
